    ...
    async def search(
        ...
        if color_id is not None and size_id is not None:
            ...
            if color_value and size_value:
                text_variant_match: Dict[str, Any] = {
                    "color": {
                        "$regex": (
                            "^" + re.escape(color_value) + "$"
                        ),
                ...
                if filters.in_stock_only:
                    text_variant_match["stock"] = {"$gt": 0}
                variant_pair_conditions.append(
                    {"variants": {"$elemMatch": text_variant_match}}
                )
            ...
            if filters.in_stock_only:
                canonical_variant_match["stock"] = {"$gt": 0}

            variant_pair_conditions.append(
                {
                    "variants": {
                        "$elemMatch": canonical_variant_match
                    }
                }
            )
